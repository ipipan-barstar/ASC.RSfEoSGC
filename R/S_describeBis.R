...
z = which(So >= tr)
x = (z - 1) %% dim(S)[1] + 1; y = (z - x) / dim(S)[1]; zl = which(So <= tl)
xl = (zl - 1) %% dim(S)[1] + 1; yl = (zl - xl) / dim(S)[1];

dg = abs(xl - yl) < 2
xl[dg] = NA
yl[dg] = NA

if (length(xl) > 450) { id = sample.int(length(xl), 400)
  xl = xl[id]
  yl = yl[id] }
...
for (isPDF in c(FALSE, TRUE)) {
  ...
  if (isPDF) pdf(paste0(FIG_DIR, experiment$name, "_", experiment$figex, ".pdf"))
  ...
  for (kk in names(dst_cats)) { points(x[as.character(nnn_cats) == kk], y[as.character(nnn_cats) == kk], col = paste0("blue", min(c(which(kk == names(table(nnn_cats))), 4))), pch = 14 + min(c(which(kk == names(table(nnn_cats))), 4)))

    text(dst_cum[kk] - dst_cats[kk] / 2, dst_cum[kk] - dst_cats[kk] / 2, labels = c(kk))
    lines(lims, c(dst_cum[kk], dst_cum[kk]), col = "green"); lines(c(dst_cum[kk], dst_cum[kk]), lims, col = "green");

  }
  if (!isPDF) readline("see ."); if (isPDF) dev.off() }
...
for (isPDF in c(FALSE, TRUE)) {
  ...
  if (isPDF) pdf(paste0(FIG_DIR, experiment$name, "_", experiment$figex, ".pdf"))
  plot(sort(aDVtop100), main = paste0("top/bottom ", topy, " point similarities"), ylim = c(0, max(aDVtop100))); points(sort(aDVlow100), col = "blue3"); if (!isPDF) readline("see ."); if (isPDF) dev.off() }

experiment$figex = "toplowdiff"
for (isPDF in c(FALSE, TRUE)) {

  if (isPDF) pdf(paste0(FIG_DIR, experiment$name, "_", experiment$figex, ".pdf"))
  plot(sort(aDVtop100 - aDVlow100), main = "average difference top bottom mean", ylab = "point similarity 100 span", ylim = c(0, max(c(aDVtop100 - aDVlow100)))); if (!isPDF) readline("see ."); if (isPDF) dev.off() }
