...
kmeansWITHweights <- function(x, centers, iter.max = 10L, nstart = 1L, algorithm = c("Hartigan-Wong", "Lloyd", "Forgy", "MacQueen"), trace = FALSE, theWeights = NULL)
{
  ...
  for (run in 2L:nstart)
  {
    ...
    print(c(run, fit$tot.withinss, fit0$tot.withinss))
    flush.console();
